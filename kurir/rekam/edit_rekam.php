...
<?php $ambil = $koneksi->query("SELECT * FROM rekam WHERE id_rekam = '$_GET[id]'");
 ...
 ?>
 <?php $ambil = $koneksi->query("SELECT * FROM obat order by jenis_obat asc");
...
<br> <?php if (isset($_POST['proses'])) {?>
	...
	<div class="form-group col-md-4">
		<select name="jenis_obat" id="" class="form-control">
			...
			<option value="Tube" <?php if ($pecah['jenis_obat'] == "Tube"){
					echo "selected";
					}?>>Tube</option>
				<option value="Botol" <?php if ($pecah['jenis_obat'] == "Botol"){
					echo "selected";
					}?>>Botol</option>
				<option value="Sachet" <?php if ($pecah['jenis_obat'] == "Sachet"){
					echo "selected";
					}?>>Sachet</option>
				<option value="Kaplet" <?php if ($pecah['jenis_obat'] == "Kaplet"){
					echo "selected";
					}?>>Kaplet</option>
				<option value="Tablet" <?php if ($pecah['jenis_obat'] == "Tablet"){
					echo "selected";
					}?>>Tablet</option>
		</select>
	</div>
	<div class="form-group col-md-4">
		<input type="text" name="keterangan" placeholder="keterangan" class="form-control" value="<?php echo $tampil['keterangan']?>">
	</div>
	</div>
<?php } ?>
<br> 
 <button class="btn btn-success" name="rekam">Edit</button>
